...
import { ExtendedPricedShippingOption, ExtendedShippingMethod } from "types/global"
    ...
    <div>
      ...
      <div className="flex items-start gap-x-8">
        ...
        <div className="flex flex-col w-1/3" data-testid="shipping-method-summary">
          <Text className="txt-medium-plus text-ui-fg-base mb-1">Methods</Text>
          {!!order.shipping_methods.length && (
            order.shipping_methods?.map((sm) => {
              return (
                <div key={sm.id} className="grid content-start mb-4">
                  <Text>
                    Shipped by {(sm?.shipping_option as unknown as ExtendedPricedShippingOption).store.name}
                  </Text>
                  <Text className="txt-medium text-ui-fg-subtle">
