...
import { ExtendedPriceProduct, ProductCategoryWithChildren, ProductPreviewType } from "types/global"
...
export const getStore = cache(async function (storeId?: string) {
  if (!storeId) return null
  const { store } = await fetch(`${process.env.NEXT_PUBLIC_MEDUSA_BACKEND_URL}/store/stores/${storeId}`).then((r) => r.json()).then((res) => res).catch((err) => { throw err })
...
export const getProductsList = cache(async function ({
  ...
  const transformedProducts = (products as unknown as ExtendedPriceProduct[]).map((product) => {
